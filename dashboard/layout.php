...
function mensajeGeneral($mensaje, $modulo) {
    ...
    $alerta .= '<div class="alert alert-success alert-dismissable">' . "\n";
    $alerta .= ' <i class="fa fa-check"></i>' . "\n";
    $alerta .= ' <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>' . "\n";
    $alerta .= $mensaje == 1 ? ' <b>' . $modulo . ' creado correctamente.</b>' : '<b>Error al insertar ' . $modulo . '.</b>';
    $alerta .= '</div>' . "\n";
    return $alerta;
}
